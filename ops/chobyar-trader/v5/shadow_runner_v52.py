...
def run(app_dir: Path) -> dict[str, Any]:
    ...
    original_read_last_cycle = base.read_last_cycle
    restore_sources = public_source_fallbacks.install(base)
    selected_cycle: dict[str, Any] | None = None

    def capture_cycle(path: Path) -> dict[str, Any]:
        nonlocal selected_cycle
        cycle = original_read_last_cycle(path)
        selected_cycle = dict(cycle)
        return cycle

    def enhanced(ctx):
        if selected_cycle is None:
            raise council_evidence.EvidenceError("source cycle was not captured")
        evaluated_at = time.time()
        raw = original_council(ctx)
        result = enhance_council(ctx, raw, scorecard, now_ts=evaluated_at)
        council_evidence.record_evaluation(
            app_dir / "logs" / council_evidence.DEFAULT_JOURNAL_NAME,
            ctx,
            selected_cycle,
            scorecard,
            evaluated_at,
    ...
    base.read_last_cycle = capture_cycle
    base.run_council = enhanced
    try:
        return base.run(app_dir)
    finally:
        base.read_last_cycle = original_read_last_cycle
