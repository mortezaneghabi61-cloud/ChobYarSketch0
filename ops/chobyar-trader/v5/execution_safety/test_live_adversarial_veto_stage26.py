...
class FakeHttp:
    def __init__(self, *, divergent=False, fail_sources=()):
    ...
    def get(self, url, **kwargs):
        ...
        if "coinbase" in url:
            if "coinbase" in self.fail_sources:
                return Resp(503, {})
            return Resp(200, {"price": "79940"})
        if "kraken" in url:
            if "kraken" in self.fail_sources:
                return Resp(503, {})
            return Resp(200, {"error": [], "result": {"XXBTZUSDT": {"c": ["79960", "1"]}}})
        if "okx" in url:
            if "okx" in self.fail_sources:
                return Resp(503, {})
            return Resp(200, {"code": "0", "data": [{"last": "82000" if self.divergent else "79955"}]})
        if "bybit" in url:
            if "bybit" in self.fail_sources:
                return Resp(503, {})
...
class Stage26Tests(unittest.TestCase):
    ...
    def test_third_consecutive_sample_can_allow_four_unique_sources(self):
        with tempfile.TemporaryDirectory() as td:
            state, history = Path(td) / "s.json", Path(td) / "h.json"
            _, _, third = warm_three(FakeHttp(), state, history)
            self.assertTrue(third["allowed"])
            self.assertEqual(third["price_quorum"], 4)
            self.assertEqual(len(set(third["source_ids"])), 4)
            self.assertGreaterEqual(third["history_samples"], 3)

    def test_one_source_failure_still_allows_three_unique_after_warmup(self):
        with tempfile.TemporaryDirectory() as td:
            state, history = Path(td) / "s.json", Path(td) / "h.json"
            _, _, third = warm_three(FakeHttp(fail_sources={"bybit"}), state, history)
            self.assertTrue(third["allowed"])
            self.assertEqual(third["price_quorum"], 3)

    def test_two_source_failures_fail_closed(self):
        with tempfile.TemporaryDirectory() as td:
            state, history = Path(td) / "s.json", Path(td) / "h.json"
            _, _, third = warm_three(FakeHttp(fail_sources={"bybit", "okx"}), state, history)
            self.assertFalse(third["allowed"])
            self.assertIn("global_price_quorum_insufficient", third["flags"])

    def test_divergent_source_dispersion_blocks(self):
        with tempfile.TemporaryDirectory() as td:
            state, history = Path(td) / "s.json", Path(td) / "h.json"
    ...
    def test_duplicate_sources_in_state_block(self):
        with tempfile.TemporaryDirectory() as td:
            state = Path(td) / "s.json"
            state.write_text(json.dumps({
                ...
                "allowed": True,
                "price_quorum": 3,
                "source_ids": ["coinbase", "coinbase", "kraken"],
                "history_samples": 3,
            }))
            with self.assertRaisesRegex(Stage26Error, "stage26_duplicate_sources"):
    ...
    def test_unapproved_source_in_state_blocks(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            state.write_text(json.dumps({
                ...
                "allowed": True,
                "price_quorum": 3,
                "source_ids": ["coinbase", "kraken", "evil-source"],
                "history_samples": 3,
            }))
            with self.assertRaisesRegex(Stage26Error, "stage26_unapproved_source"):
                enforce_live_buy_veto(state_path=state, now=NOW)

    def test_quorum_must_exactly_match_unique_source_count(self):
        with tempfile.TemporaryDirectory() as td:
            state = Path(td) / "s.json"
            state.write_text(json.dumps({
                "version": 1,
            ...
            with self.assertRaisesRegex(Stage26Error, "stage26_quorum_source_mismatch"):
                enforce_live_buy_veto(state_path=state, now=NOW)
