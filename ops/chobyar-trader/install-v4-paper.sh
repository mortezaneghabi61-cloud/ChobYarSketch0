...
curl -fsS "http://127.0.0.1:${port}/public-report" -o "$work/public-report.json" || die "public report unavailable"
"$VENV/bin/python" -c 'import json,sys; d=json.load(open(sys.argv[1])); assert d.get("ok") is True and d.get("public_report") is True and d.get("live_locked") is True; assert not ({"paper","market","risk","security"} & set(d))' "$work/public-report.json" || die "public report failed sanitization validation"
