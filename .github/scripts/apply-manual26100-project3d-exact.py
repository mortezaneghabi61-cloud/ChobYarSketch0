    ...
    try {
        ...
        for(TopExp_Explorer ex(shape,TopAbs_EDGE);ex.More();ex.Next(),++edgeIndex){
            ...
            if(type==GeomAbs_Line){
                ...
                kind=std::abs(span-2.0*PI)<1.0e-6?2:3;
            }
            const double orientation=edge.Orientation()==TopAbs_REVERSED?-1.0:1.0;
            const double rec[RECORD]={
                static_cast<double>(kind),static_cast<double>(edgeIndex),
                p1.X(),p1.Y(),p1.Z(),p2.X(),p2.Y(),p2.Z(),
                center.X(),center.Y(),center.Z(),normal.X(),normal.Y(),normal.Z(),
                radius,first,last,orientation
            };
...
'''
        ...
        for(double[] d:batches){
            ...
            for(int i=0;i+n-1<d.length;i+=n){
                ...
                if(kind==NativeBRepKernel.OCCT_EDGE_LINE){
                    ...
                    if(!undoSaved){coreSaveUndo();undoSaved=true;}
                    coreAddConstructionLine(a.x,a.y,b.x,b.y);lines++;continue;
                }
                if(kind==NativeBRepKernel.OCCT_EDGE_CIRCLE||kind==NativeBRepKernel.OCCT_EDGE_ARC){
                    ...
                    // Orthogonal projection of a tilted circle is an ellipse. Do not fake it as a circle/polyline.
                    ...
                    if(kind==NativeBRepKernel.OCCT_EDGE_CIRCLE){
                        ...
                        if(!undoSaved){coreSaveUndo();undoSaved=true;}
                        coreAddConstructionCircle(c.x,c.y,radius);circles++;continue;
                    }
                    ...
                    if(!undoSaved){coreSaveUndo();undoSaved=true;}
                    coreAddConstructionArc(c.x,c.y,radius,start,sweep);arcs++;continue;
                }
                unsupported++;
            }
        }
        if(undoSaved)invalidate();
        return "Project 3D • Line "+lines+" • Circle "+circles+" • Arc "+arcs
                +" • Unsupported "+unsupported+" • Skipped "+skipped;
    }

    private static String projectLineKey(PointF a,PointF b){
        String x=projectPointKey(a),y=projectPointKey(b);return x.compareTo(y)<=0?x+"|"+y:y+"|"+x;
    }
    ...
    private static String projectArcKey(PointF c,float r,float start,float sweep){
        return projectCircleKey(c,r)+":"+Math.round(start*100f)+":"+Math.round(sweep*100f);
    }
