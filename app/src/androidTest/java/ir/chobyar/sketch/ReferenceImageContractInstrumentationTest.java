...
public final class ReferenceImageContractInstrumentationTest {
    ...
    public void imageAttachesToActiveSketchPlaneWithPhysicalAspectRatio() throws Exception {
        ...
        try (ActivityScenario<ChobYarActivity> scenario = ActivityScenario.launch(ChobYarActivity.class)) {
            ...
            scenario.onActivity(activity -> {
                ...
                Log.i(TAG, "REFERENCE_IMAGE_ATTACH_RESULT plane=" + expectedPlane + " width=100.0 height=50.0 overview=true");
            });
        }
    }
    ...
    public void cameraViewChangesKeepImageRegisteredToTheSamePlane() throws Exception {
        ...
        try (ActivityScenario<ChobYarActivity> scenario = ActivityScenario.launch(ChobYarActivity.class)) {
            ...
            scenario.onActivity(activity -> {
                ...
                assertSame(planeBefore, objectField(referenceImage(canvas), "plane"));
                Log.i(TAG, "REFERENCE_IMAGE_CAMERA_RESULT registered=true views=ISO,RIGHT,TOP plane=" + stringField(planeBefore, "label"));
            });
        }
    }
    ...
    private static Object referenceImage(SpatialCadCanvasView canvas) {
        ...
        } catch (Exception e) { throw new AssertionError("Could not inspect reference image", e); }
    }
    private static Object objectField(Object target, String name) {
        ...
        catch (Exception e) { throw new AssertionError("Could not inspect field " + name, e); }
    }
    private static float floatField(Object target, String name) {
        try { Field field = target.getClass().getDeclaredField(name); field.setAccessible(true); return field.getFloat(target); }
        catch (Exception e) { throw new AssertionError("Could not inspect float field " + name, e); }
    }
    private static boolean booleanField(Object target, String name) {
        ...
        catch (Exception e) { throw new AssertionError("Could not inspect boolean field " + name, e); }
    }
    private static String stringField(Object target, String name) { Object value = objectField(target, name); return value == null ? null : value.toString(); }
    private static void setFloatField(Object target, String name, float value) {
        try { Field field = target.getClass().getDeclaredField(name); field.setAccessible(true); field.setFloat(target, value); }
        catch (Exception e) { throw new AssertionError("Could not set float field " + name, e); }
    }
    private static void setBooleanField(Object target, String name, boolean value) {
        ...
        catch (Exception e) { throw new AssertionError("Could not set boolean field " + name, e); }
    }
    private static float imageHeightMm(Object image) {
        try { Method method = image.getClass().getDeclaredMethod("heightMm"); method.setAccessible(true); return ((Number) method.invoke(image)).floatValue(); }
        catch (Exception e) { throw new AssertionError("Could not compute reference image height", e); }
    }
}
