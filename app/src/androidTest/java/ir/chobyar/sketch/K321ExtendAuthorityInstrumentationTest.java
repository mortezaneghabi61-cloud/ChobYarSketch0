...
/** K3.21 RED fence for model-owned Extend authority. */
@RunWith(AndroidJUnit4.class)
public final class K321ExtendAuthorityInstrumentationTest {
    private static final double EPS = 1.0e-4;

    @Test public void extendMustCommitStableIdsIntoModelWithoutMirrorResync() throws Exception {
        onMain(() -> {
            K33MirroredCadCanvasView cad = separatedCanvas();
            ...
            assertTrue(cad.extendSelectedLines().contains("Extend"));
            ...
            assertLine(lines.get(1), 10, 0, 10, 10);
            return true;
        });
    }

    @Test public void extendUndoRedoMustRoundTripBothLinesAsOneModelHistoryStep() throws Exception {
        onMain(() -> {
            K33MirroredCadCanvasView cad = separatedCanvas();
            List<String> ids = modelIds(cad);
            List<SketchGeometry.Line> before = modelLines(cad);

            cad.extendSelectedLines();
            cad.requireSketchMirrorParity();
            List<SketchGeometry.Line> extended = modelLines(cad);
            assertFalse(sameGeometry(before, extended));
            ...
            cad.requireSketchMirrorParity();
            assertEquals(ids, modelIds(cad));
            assertGeometry(extended, modelLines(cad));
            assertTrue(cad.sketchAuthorityHistoryActive());
            return true;
        });
    }

    @Test public void extendMustPreserveExistingConstraintStableIds() throws Exception {
        onMain(() -> {
            K33MirroredCadCanvasView cad = separatedCanvas();
    ...
    @Test public void parallelExtendMustBeTrueNoOpWithoutMirrorResyncOrHistoryStep() throws Exception {
        onMain(() -> {
            ...
            String out = cad.extendSelectedLines();
            cad.requireSketchMirrorParity();
    ...
    private static K33MirroredCadCanvasView separatedCanvas() {
        K33MirroredCadCanvasView cad = canvas();
        CadCanvasView.Entity a = createLine(cad, 0, 0, 4, 0);
        CadCanvasView.Entity b = createLine(cad, 10, 5, 10, 10);
        selectPair(cad, a, b);
        return cad;
    }
