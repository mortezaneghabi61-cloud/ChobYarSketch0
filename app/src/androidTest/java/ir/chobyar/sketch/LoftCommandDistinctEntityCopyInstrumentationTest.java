...
/** Exact user-visible LOFT3D same-entity copy contract. */
@RunWith(AndroidJUnit4.class)
public final class LoftCommandDistinctEntityCopyInstrumentationTest {

    @Test public void loft3dRequiresDistinctProfileEntityNumbers() {
        try(ActivityScenario<ChobYarActivity> scenario=ActivityScenario.launch(ChobYarActivity.class)){
            scenario.onActivity(activity->{
                ...
                assertEquals(
                        "LOFT3D — Profiles must use different entity numbers",
                        canvas.createLoftByEntityIndex(1,1)
                );
            });
        }
    }
